...
//Modified by Dai Griffiths, Dragonflight Design, St.Patrick's Day 2004
//	- Spelling errors corrected
//	- Mouse flags corrected so that the helpid can be inserted in the main string
//	- Full backwards compilation compatibility with existing FS2K2/FS2K gauges added
//	- All Arne Bartels' original hard work on correcting the previous two SDK gauges.h files added
//	- All Arne's excellent shorthand conversion macros added
//	- Hopefully all previous third-party gauges based on fs2k2gauges.h will now compile
...
#define PDDF_ALTITUDE	 	BIT0	// Altitude hold turned on
#define PDDF_VELOCITY_ALTITUDE	BIT1	// Vertical speed hold turned on (or alt)
#define PDDF_PITCH		BIT2	// Pitch hold turned on (or vs or alt)
#define PDDF_BANK		BIT3	// Bank hold turned on (or heading)
#define PDDF_HEADING		BIT4	// Heading hold turned on
#define PDDF_ROLL_RATE		BIT5	// Roll rate hold turned on
#define PDDF_BODY_PITCH_VEL	BIT6	// Body pitch velocity turned on
#define PDDF_AIRSPEED	 	BIT7	// Airspeed hold turned on
#define PDDF_GROUNDSPEED	BIT8	// Groundspeed hold
#define PDDF_POSITION	 	BIT9	// Ground position hold or formation flight position hold
#define PDDF_GROUND_VEL_DIR	BIT10	// Additional flag to heading control - ground velocity direction hold


#define AUTOPILOT_ALL_ALTITUDE_FLAGS 		(PDDF_ALTITUDE | PDDF_VELOCITY_ALTITUDE | PDDF_PITCH | PDDF_BODY_PITCH_VEL)
#define AUTOPILOT_ALL_HEADING_FLAGS		(PDDF_HEADING | PDDF_BANK | PDDF_ROLL_RATE | PDDF_POSITION)

#define AUTOPILOT_ENABLE_ALTITUDE_HOLD	(PDDF_ALTITUDE | PDDF_VELOCITY_ALTITUDE | PDDF_PITCH)
#define AUTOPILOT_ENABLE_PITCH_HOLD		(PDDF_PITCH)
#define AUTOPILOT_ENABLE_BODY_PITCH_VEL_HOLD	(PDDF_BODY_PITCH_VEL)
#define AUTOPILOT_ENABLE_HEADING_HOLD	(PDDF_HEADING | PDDF_BANK)
#define AUTOPILOT_ENABLE_BANK_HOLD		(PDDF_BANK)
#define AUTOPILOT_ENABLE_ROLL_RATE_HOLD 	(PDDF_ROLL_RATE)
#define AUTOPILOT_ENABLE_POSITION_HOLD	(PDDF_POSITION | PDDF_HEADING)
...
// Vor transmitter type for fs2k2gauges.h compatibility
#define VOR_UNKNOWN	0
#define VOR_TERMINAL	1
#define VOR_LOW_ALT	2
#define VOR_HIGH_ALT	3
#define VOR_ILS		4
...
#define    SET_GAUGE_BRIGHT( element)    element->image_flags |= IMAGE_USE_BRIGHT
#define    SET_GAUGE_NOT_BRIGHT( element)    element->image_flags &= ~IMAGE_USE_BRIGHT 
...
{														\
	UINT32	index;												\
	if(pelement==NULL)	/*invalid start*/									\
	return	NULL;												\
	for(index=0;index<pos_element;index++)	/*advance pointer to desired element*/					\
	{													\
	if(pelement->next_element==NULL)	/*stop if linked list is ending*/						\
	return	NULL;												\
...
EXTERN_C void FSAPI	add_imagedata_to_listelement(PELEMENT_HEADER	pelement,UINT32	pos_element,FLAGS image_flags)	\
{															\
	if((pelement=get_listelement_pointer(pelement,pos_element))==NULL)						\
	return;													\
